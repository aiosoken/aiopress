...
/**
 * エージェント実行Cloud Function
 */
export const runAgent = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    const { brandId, message, sessionId } = data;
        ...
        for (const part of functionCalls) {
          ...
          try {
            ...
            if (result?.creativeId) {
              creativeIds.push(result.creativeId);
            }
 ...
 * エージェントセッション一覧取得
 */
export const getAgentSessions = functions
  .region("asia-northeast1")
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 * エージェントセッション詳細取得
 */
export const getAgentSession = functions
  .region("asia-northeast1")
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
